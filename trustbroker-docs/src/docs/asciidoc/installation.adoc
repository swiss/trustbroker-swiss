...
| ``postgresql`` providing session and token storage.
| docker exec -it postgresql psql --host=localhost --port=5432 --dbname=tbss16 --username=root --password
| Investigate storage: SET search_path TO tbss16; \dt; \d tb_auth_session_cache;

| ``mariadb`` providing session and token storage as alternative to postgresql.
| docker exec -it mariadb mysql --protocol=tcp --host=localhost --port=3306 --database=TBSS16 --user=root --password=secret
...
| Documentation: https://swiss.github.io/trustbroker.swiss/

| ``trustbroker-samlmock`` simulating a SAML RP and CP to play around with.
| RP entry points: http://localhost:7070
| Mock service simulating RP and CP messages based on the ``gitops.trustbroker.swiss/trustbroker-samlmock data`` to play around with the setup.
|===
...
- Clients: You can in general map a JKS into the containers ``/usr/lib/jvm/zulu21-ca/lib/security/cacerts``
...
- Download https://github.com/swiss/trustbroker.swiss/blob/main/trustbroker-install/docker/docker-compose.yml[docker-compose.yml]
- Download https://github.com/swiss/trustbroker.swiss/blob/main/trustbroker-install/env-vault.conf[env-vault.conf]
- Download https://github.com/swiss/trustbroker.swiss/blob/main/trustbroker-install/run.sh[run.sh]
...
For trials, you can use the Gitea server as shown in the docker-compose setup, you need to switch from the github to the gitea URL for the gitops.trustbroker.swiss repository..
...
- Create a repository ``gitops.trustbroker.swiss``.
...
[source,bash]
----
git clone -b main https://github.com/swiss/gitops.trustbroker.swiss.git
cd gitops.trustbroker.swiss
----
...
[source,bash]
----
git remote set-url origin ssh://git@localhost:222/trustbroker-swiss/gitops.trustbroker.swiss.git
...
[source,bash]
----
git remote set-url origin http://git@localhost:3000/trustbroker-swiss/gitops.trustbroker.swiss.git
git push
----

Run the services in the directory where you stored the tweaked ``docker-compose.yml`` and ``env-vault.conf`` using the following
https://github.com/swiss/trustbroker.swiss/trustbroker-install/docker/run.sh[run.sh]:
